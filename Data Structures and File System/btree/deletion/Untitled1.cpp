...
void del(btnode *&L, int data)
{
	if (L!=NULL)
	{
		int i;
		for (i=0;i<2*d&&L->key[i]<data&&L->key[i]!=-1;i++);
		if (L->key[i]!=data)
		del(L->child[i],data);
		else
		{
			...
			else
			{
				...
				L->key[i]=temp->key[temp->count-1];
				del(L->child[i], L->key[i]);
			}
		}
		if (L->count<d&&L->parent!=NULL)
		{
			int index=self_index(L);
			if (index!=0&&L->parent->child[index-1]->count!=d)
			{                 
	            //key redist from left
				btnode *T;
				T=L->parent->child[index-1];
				for (i=L->count;i>0;i--)
				{
					L->key[i]=L->key[i-1];
					L->child[i+1]=L->child[i];
				}
				L->child[1]=L->child[0];
				L->key[0]=L->parent->key[index-1];
				L->parent->key[index-1]=T->key[T->count-1];
				L->child[0]=T->child[T->count];
			...
			else if (index!=L->parent->count&&L->parent->child[index+1]->count!=d)
			{
				//key redist from right
				btnode *T;
				T=L->parent->child[index+1];
				L->key[L->count]=L->parent->key[index];
				L->child[L->count+1]=T->child[0];
				if (L->child[L->count+1]!=NULL)
				L->child[L->count+1]->parent=L;
				L->parent->key[index]=T->key[0];
				for (i=0;i<T->count-1;i++)
				{
					T->key[i]=T->key[i+1];
					T->child[i]=T->child[i+1];
				}
				T->child[T->count-1]=T->child[T->count];
				T->child[T->count]=NULL;
				T->key[T->count-1]=-1;
				T->count--;
				L->count++;
			}
			else if (index==L->parent->count)
			{
				//node binding to with parent->child[index-1]
				btnode *T;
				T=L->parent->child[index-1];
				T->key[T->count++]=T->parent->key[index-1];
				T->child[T->count]=L->child[0];
				if (T->child[T->count]!=NULL)
				T->child[T->count]->parent=T;
				for (i=0;i<d-1;i++)
				{
					T->key[T->count++]=L->key[i];
					T->child[T->count]=L->child[i+1];
					if (T->child[T->count]!=NULL)
					T->child[T->count]->parent=T;
				}
				for (i=index-1;i<L->parent->count-1;i++)
				{
					L->parent->key[i]=L->parent->key[i+1];
					L->parent->child[i+1]=L->parent->child[i+2];
				}
				L->parent->key[L->parent->count-1]=-1;
				L->parent->child[L->parent->count]=NULL;
				T->parent->count--;
			}
			else
			{
				//node binding with parent->child[index+1]
				btnode *T;
				T=L->parent->child[index+1];
				L->key[L->count++]=L->parent->key[index];
				L->child[L->count]=T->child[0];
				if (L->child[L->count]!=NULL)
				L->child[L->count]->parent=L;
				for (i=0;i<d;i++)
				{
					L->key[L->count++]=T->key[i];
					L->child[L->count]=T->child[i+1];
					if (L->child[L->count]!=NULL)
					L->child[L->count]->parent=NULL;
				}
				for (i=index;i<L->parent->count-1;i++)
				{
					L->parent->key[i]=L->parent->key[i+1];
					L->parent->child[i+1]=L->parent->child[i+2];
				}
				L->parent->key[L->parent->count-1]=-1;
				L->parent->count--;
				L->parent->child[L->parent->count+1]=NULL;
			}
		}
		else if (L->parent==NULL&&L->count==0)
		L=L->child[0];
	}
