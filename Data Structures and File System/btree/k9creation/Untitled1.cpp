...
bool krd_left(btnode *T,int index,btnode *temp, bool abc)
{
	if (index!=1&&T->parent->child[index-1]->count==2*d)
	{
		int ind=self_index(T->parent->child[index-1]);
		if (krd_left(T->parent->child[index-1],ind,temp,false)) return true;
		else return false;		
	}
	if (index!=0&&T->parent->child[index-1]->count<2*d&&abc==false)
	{
		btnode *temp2=T->parent->child[index-1];
		temp2->count++;
		int i=0;
		while (i<=2*d||temp2->key[i]!=-1) i++;
		while (i!=0)
		{
		...
		temp2->key[0]=temp2->parent->key[index-1];
		temp2->parent->key[index-1]=T->key[2*d-1];
	...
	if (index!=0&&T->parent->child[index-1]->count<2*d&&abc==true)
	{
		int i;
		int m[2*d+1];
		for (i=0;i<2*d;i++)
...
bool krd_right(btnode *T, int index, btnode *temp, bool add)
{
	if (index!=2*d-1&&T->parent->child[index+1]->count==2*d)
	{
		int ind=self_index(T->parent->child[index+1]);
		if (krd_right(T->parent->child[index+1],ind,temp,false)==false) return false;
	}
	if (T->parent->child[index+1]!=NULL&&T->parent->child[index+1]->count<2*d&&add==false)
	{
		btnode *temp2=T->parent->child[index+1];
		temp2->count++;
		int i=0;
		...
		temp2->parent->key[index]=T->key[2*d-1];
		temp2->child[0]=T->child[2*d];
		T->child[2*d]=NULL;
		T->count--;
		T->key[2*d-1]=-1;
	...
	if (index!=2*d&&T->parent->child[index+1]!=NULL&&add==true)
	{
		int i;
		int m[2*d+1];
		for (i=0;i<2*d;i++)
	...
	if (T!=NULL)
	{
		...
		if (temp!=NULL)
		{
			...
			else if (T->count==2*d)
			{
				...
				bool abc;
				if (T->parent!=NULL&&index!=2*d&&T->parent->child[index+1]!=NULL)
				{
					abc=krd_right(T,index,temp,true);
					if (abc)
					return NULL;
				}
				if (!abc&&(T->parent!=NULL&&index!=0))
				{
					abc=krd_left(T,index,temp,true);
					if (abc)
					return NULL;
				}
				if (!abc)
