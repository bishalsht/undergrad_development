...
void printPacket(const u_char *packet,int length){
  ...
  for (unsigned int i = 0; i < length; i++) {
    if (i%LEN == 0) {
      if (i > 0) {
        cout<< "  ";
        for (int j = 0; j < LEN; j++) cout<< ascii[j];
      }
      sprintf(outstr, "%.4d: ", i);
      cout << endl << outstr;
    }
    sprintf(outstr, "%.2x ", (unsigned char)packet[i]);
    cout<< outstr;
    ascii[i%LEN] = ((packet[i] >= 32 && packet[i] <= 126) ? packet[i] : '.');
  }
  for (int i = LEN - length % LEN; i > 0; i--) cout<< "   ";
  cout<< "  ";
  for (unsigned int j = 0; j < length % LEN; j++) cout<< ascii[j];
  cout<<"\n";
}
