...
int main(int argc, char *const *argv)
{
    ...
    int z;
    for(z=0;z<SERVICES;z++)
    {
        if((sock[z]=socket(AF_INET,SOCK_STREAM,0))==-1)
        {
        ...
        server.sin_port=htons(atoi(argv[1])+z);
        server.sin_addr.s_addr=INADDR_ANY;
        bzero(&server.sin_zero,8);

        if(bind(sock[z],(struct sockaddr *)&server,sizeof(struct sockaddr_in))==-1)
        {
            perror("bind : ");
            exit(-1);
        }

        if((listen(sock[z],MAX_CLIENTS))==-1)
        {
            perror("listen : ");
            exit(-1);
        }
    ...
    {
        int m, i;
        for (m = 0; m < SERVICES; ++m)
            FD_SET(sock[m], &rfds);

        select(sock[SERVICES-1]+1, &rfds, NULL, NULL, NULL);

        for(i = 0; i < SERVICES; ++i)
        {
            if (FD_ISSET(sock[i], &rfds))
            {
                if((new=accept(sock[i],(struct sockaddr *)&client,&sockaddr_len))==-1)
                {
                    perror("accept : ");
                    exit(-1);
                }
